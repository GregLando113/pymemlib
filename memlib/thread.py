...
class ProcessThread(object):
    ...
    def alive(self):
        ...
        code = win32.DWORD()
        success = win32.GetExitCodeThread(self.handle, win32.byref(code))
        if not success:
            raise win32.Win32Exception()
    ...
    def context(self, flags=win32.CONTEXT_FULL):
        ...
        success = win32.GetThreadContext(self.handle, win32.byref(context))
        if not success:
            raise win32.Win32Exception()
        return context
    ...
    def teb(self):
        ...
        ntstatus = win32.NtQueryInformationThread(
            self.handle, win32.ThreadBasicInformation, win32.byref(info), win32.sizeof(info), None
        )
